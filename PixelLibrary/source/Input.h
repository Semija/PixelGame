...
int GetMouseX() {
	...
	return (int)(X / ((double)realWindowWidth / (double)windowWidth));
}

int GetMouseY() {
	double Y = 0;
	glfwGetCursorPos(window, NULL, &Y);
	return (int)(Y / ((double)realWindowHeight / (double)windowHeight));
}
...
bool IsMousePressed(int mouseKey) {
	return (glfwGetMouseButton(window, mouseKey) == GLFW_PRESS);
}

bool IsMouseDown(int mouseKey) {
	return (glfwGetMouseButton(window, mouseKey) == GLFW_REPEAT);
}

bool IsMouseReleased(int mouseKey) {
	return (glfwGetMouseButton(window, mouseKey) == GLFW_RELEASE);
}
